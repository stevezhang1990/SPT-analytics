...
i = 0
#P: power output, t is the time for exhaustion
#P and CP are power (speed) and critical power 
def CP_model_raw(v,t,CP):
    v_masked = np.ma.masked_invalid(v)
    v_filt = np.ma.masked_where(v<=0,v)
    work_rate = (v-CP)*t
    return work_rate
#t: total endurance time, Pw and Pr: work, rest power
#Tw and Tr: work and rest interval time, tau_w: reconstitution of work rate
def CP_model_inter(CP,work_rate,v,v_crt_per,t_sample)
    u = t_sample[1]-t_sample[0]
    v_crt = v_crt_per*np.max(v)
    tw = np.mean(np.ma.masked_where(v<v_crt,t_sample)
    tr = np.mean(np.ma.masked_where(v>v_crt,t_sample)
    Pw = np.mean(np.ma.masked_where(v<v_crt,v)
    Pr = np.mean(np.ma.masked_where(v>v_crt,v)
    t = n*(tw+tr)+work_rate-n*((Pw-CP)*tw-(CP-Pr)*tr)/(Pw-CP)
    D_cp = v-CP
    tau_w = 546*exp(-0.01*D_cp)+316
    work_rate_bal = work_rate - integral.quad(D_cp*exp(-(t-u)/tau_w),0,t)
    return work_rate_bal
