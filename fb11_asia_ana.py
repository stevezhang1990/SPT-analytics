...
df = pd.read_pickle(path1+'11_odds_history_v2.csv')
data_raw = df.loc[df['Season']>=2010.5 & df['Season']<=2016.5]
    ...
    for kk in range(2):
        ...
        if li != None and ci>=0:
            ...
            if data_raw.iat[i,4]>=0 and data_raw.iat[i,5]>=0:
                if data_raw.iat[i,4] - data_raw.iat[i,5] +AHsize > 0:
                    data2_obs[kk,ci,li,di,counter[kk,ci,li,di],0] = 1
                elif abs(data_raw.iat[i,4] - data_raw.iat[i,5])<=0.25:
